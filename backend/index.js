    ...
    app.get('/api/customers', async (req, res) => {
      try {
        ...
        // Fetch order counts for each customer manually
        ...
        if (customerIds.length > 0) {
          ...
          orderCountMap = allOrders ? allOrders.reduce((map, order) => {
            map[order.user_id] = (map[order.user_id] || 0) + 1;
            return map;
          }, {}) : {};
          console.log('Order counts:', orderCountMap);
        } else {
          console.log('No customer IDs to query orders for.');
        }
